# weaveOS — MCP agent example

End-to-end demo of an agent using the weaveOS Streamable HTTP MCP server.
...
3. **`tools/call register_agent`** — list the calling agent in the
   weaveOS marketplace (`/agents` shows it immediately).
...
1. initialize
  ↳ session: 7e0e52a9…
   ✓ server: weaveos v0.1.0
   ✓ protocol: 2025-03-26

2. tools/list
   ✓ 2 tools available:
     • register_agent — Register the calling agent in the weaveOS marketplace…
     • start_workflow — Run a full weaveOS workflow lifecycle…
...
Then talk to Claude: *"register an agent in weaveOS that summarises PDFs"* — it
...
weaveOS *could* ship an SDK and tell every agent author to install it. MCP
flips the integration: agents add weaveOS as a *skill* in their existing
