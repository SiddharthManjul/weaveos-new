#!/usr/bin/env node
// End-to-end MCP agent demo for weaveOS.
//
...
//
// Usage:
//   WEAVEOS_API_KEY=wos_... node examples/mcp-agent.mjs [baseUrl]
//
// Default base URL: http://localhost:3000

const BASE = (process.argv[2] ?? "http://localhost:3000").replace(/\/$/, "");
const KEY = process.env.WEAVEOS_API_KEY;

if (!KEY) {
  console.error("✗ set WEAVEOS_API_KEY (mint one at " + BASE + "/developer)");
  process.exit(1);
}
...
async function main() {
  ...
  const reg = await rpc("tools/call", {
    ...
    arguments: {
      ...
      description:
        "Demonstration agent that the weaveOS MCP example script registered automatically. Closes pretend tickets, charges 0.05 SUI per run.",
