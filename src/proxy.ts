...
//     callback), and /api/* (the callback page POSTs to /api/auth/zklogin/prove
//     before the cookie exists).
//   • Visiting an authenticated route without the `weaveos.user` cookie
...
export function proxy(request: NextRequest): NextResponse {
  ...
  const cookie = request.cookies.get(WEAVEOS_USER_COOKIE);
