...
const STAGES: StageDef[] = [
  { key: "quote",          title: "Quote",                 hint: "Customer commits price + criteria, frozen on chain",                  icon: LegalDocumentIcon  },
  { key: "workflow",       title: "Payment + Workflow",    hint: "PTB locks USDC/SUI escrow + creates the Workflow object",            icon: WorkflowCircleIcon },
  { key: "execution",      title: "Execution",             hint: "Agent runs off-chain; SDK posts cost items + trace blob",            icon: FileSearchIcon     },
  { key: "verify",         title: "Verifier (Vercel)",     hint: "DSL eval, Walrus upload, BCS encode, ed25519 sign — your /api/verify", icon: Settings01Icon     },
  { key: "outcome",        title: "Outcome on chain",      hint: "Move verifies ed25519 sig vs registered pubkey, creates Outcome",    icon: CheckmarkCircleIcon },
  { key: "dispute_window", title: "Dispute window",        hint: "Customer can challenge here. 10s for demo (24-168h in prod)",       icon: HourglassIcon       },
  { key: "settle",         title: "Atomic settlement",     hint: "Single PTB pays all 4 parties — no partial state possible",          icon: CoinsDollarIcon    },
];
...
export function LifecycleDemoDrawer({
  ...
  const [signingMode, setSigningMode] = useState<"zklogin" | "ed25519" | null>(null);
    ...
    (async () => {
      try {
        ...
        const resp = await fetch("/api/demo/run-lifecycle", {
          ...
          body: JSON.stringify({ outcomeMode, zkLoginSession }),
  ...
  function handleEvent(msg: { event: string; data: Record<string, unknown> }) {
    if (msg.event === "start") {
      setSigningMode((msg.data.signingMode as "zklogin" | "ed25519") ?? "ed25519");
        ...
        {signingMode && customerAddr && (
          <div
            ...
            style={{
              ...
              border: `1px solid ${
                signingMode === "zklogin" ? "#1e3a8a" : "#272727"
              }`,
            }}
          >
            <span
              className="font-semibold uppercase tracking-wider"
              style={{ color: signingMode === "zklogin" ? "#60a5fa" : "#a3a3a3" }}
            >
              {signingMode === "zklogin" ? "zkLogin" : "platform key"}
            </span>
            ...
            {signingMode === "zklogin" && (
              <span className="ml-auto text-[#60a5fa]">
                signed by your Google identity
              </span>
            )}
          </div>
        )}
        ...
        <div className="border-t border-[#1e1e1e] px-5 py-4 flex items-center gap-3 shrink-0">
          {complete ? (
            ...
            <span className="text-[12px] text-[#5a5a5a]">
              Streaming stages over NDJSON — total ~30–40s including 10s dispute window…
            </span>
          ) : error ? (
            <button
              onClick={onClose}
