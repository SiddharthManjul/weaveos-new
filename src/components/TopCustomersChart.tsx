...
export type TopCustomersDatum = {
  ...
  customer: string;
  /** GMV in SUI (post-decimal conversion). */
  gmv: number;
  /** Currently locked in escrow, in SUI. */
  escrowed: number;
};
...
function formatY(v: number) {
  // Compact display: <1 SUI uses 3 decimals, larger uses 1.
  if (v < 1) return `${v.toFixed(3)} SUI`;
  if (v < 10) return `${v.toFixed(2)} SUI`;
  return `${v.toFixed(0)} SUI`;
}
      ...
      <div className="flex items-center justify-between shrink-0 mb-3">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            />
            <span className="text-[#a3a3a3] text-[13px]">Escrowed</span>
          </div>
          <div className="flex items-center gap-2">
            <span
              className="w-4 h-2.5 rounded-sm shrink-0"
