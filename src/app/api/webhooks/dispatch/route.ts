...
//   2. Compute HMAC-SHA256(signing_secret, payload) → X-Weaveos-Signature
  ...
  try {
    const resp = await fetch(settings[0].webhookUrl, {
      ...
      headers: {
        ...
        "X-Weaveos-Event": delivery.eventType,
        "X-Weaveos-Signature": signature,
        "X-Weaveos-Delivery-Id": String(delivery.id),
      },
      body,
    });
    if (resp.ok) {
