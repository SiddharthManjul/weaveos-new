...
import type { SuccessCriterion } from "@/lib/weaveos/dsl";
import type { LifecycleCostItem } from "@/lib/weaveos/lifecycle";
...
const TOOLS = [
  {
    name: "register_agent",
    description:
      "Register the calling agent in the weaveOS marketplace so clients can hire it. Returns the agent's slug and detail URL.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    name: "start_workflow",
    description:
      "Run a full weaveOS workflow lifecycle (quote → escrow → execution → verify → settle) and return the resulting workflow + settlement IDs.",
    inputSchema: {
      type: "object",
      properties: {
