...
export const metadata = { title: "SDKs — WeaveOS Docs" };
    ...
    <DocsLayout>
      <div className="docs-body">
        ...
        <p className="mt-3" style={{ color: "#a3a3a3", fontSize: 15 }}>
          WeaveOS provides official SDKs for Node.js and Python, plus full cURL
        ...
        <h3>Install</h3>
        <Code lang="bash">npm install @weaveos/sdk</Code>

        <h3>Initialise</h3>
        <Code lang="javascript">{`import WeaveOS from "@weaveos/sdk";
...
console.log(event.data.workflowId);  // "wf_e4rgffg44fg4g44"`}
        ...
        <h3>Install</h3>
        <Code lang="bash">pip install weaveos</Code>

        <h3>Initialise</h3>
        <Code lang="python">{`import weaveos
import os

client = weaveos.WeaveOS(api_key=os.environ["WEAVEOS_API_KEY"])`}
        </Code>
...
def webhook():
  ...
  -d '{
    ...
    "payload": { "ticketId": "TKT-8821" }
        ...
        <Code lang="bash">{`curl "https://api.weaveos.dev/v1/workflows?status=Settled&limit=10" \\
  -H "Authorization: Bearer $WEAVEOS_API_KEY"`}
        </Code>

        <h3>Fetch a workflow</h3>
        <Code lang="bash">{`curl https://api.weaveos.dev/v1/workflows/wf_e4rgffg44fg4g44 \\
  -H "Authorization: Bearer $WEAVEOS_API_KEY"`}
        </Code>

        <Callout type="tip">
          Set <code>WEAVEOS_API_KEY</code> as a shell variable to avoid repeating
          it across cURL commands: <code>export WEAVEOS_API_KEY=sk_test_...</code>
        </Callout>
        ...
        <Code lang="javascript">{`import WeaveOS, { APIError, AuthenticationError, RateLimitError } from "@weaveos/sdk";
